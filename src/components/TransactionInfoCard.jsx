import {Trash2, TrendingDown, UtensilsCrossed, TrendingUp} from "lucide-react";
...
const TransactionInfoCard = ({icon, title, date, amount, type, hideDeleteBtn, onDelete}) => {
    const getAmountStyles = () => type === 'income' ? 'bg-green-500 text-green-800' : 'bg-red-50 text-red-800'
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-md ${getAmountStyles()}`}>
                        <h6 className="text-xs font-medium">
                            {type === 'income' ? '+' : '-'} {addThousandsSeparator(amount)} VNĐ
                        </h6>
                        {type === 'income' ? (
                            <TrendingUp size={15}/>
                        ) : (
